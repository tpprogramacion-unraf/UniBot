  ...
  return (
    <div style={{ padding: '44px 52px', width: '100%', boxSizing: 'border-box' }}>
      <div className="page-header">
        <div>
          <h1>Calendario</h1>
          <p className="subtitle">Eventos académicos y horarios</p>
        </div>
      ...
      {showForm && (
        <div className="form-card" style={{ marginBottom: 24 }}>
          ...
          <div className="form-grid" style={{ marginTop: 16 }}>
            ...
            <div className="field"><label>Fecha y hora</label><input type="datetime-local" value={form.date} onChange={e => setForm({ ...form, date: e.target.value })} /></div>
            ...
            {cells.map((cell, i) => {
              ...
              return (
                <div key={i} style={cellStyle(cell)} onClick={() => cell.current && setSelectedDay(new Date(year, month, cell.day))}>
                  <span style={{ fontSize: '0.78rem', fontFamily: 'Inter, sans-serif', display: 'block', marginBottom: 4, color: isT ? 'var(--accent)' : 'var(--text)', fontWeight: isT ? 700 : 400, letterSpacing: '-0.01em' }}>
              ...
              return (
                <>
                  ...
                  {dayEvs.length === 0 && daySlots.length === 0 && (
                    <p style={{ color: 'var(--text-3)', fontSize: '0.78rem' }}>No hay eventos para este día</p>
                  )}
                </>
              )
            })()}
