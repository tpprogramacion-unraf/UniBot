...
class DocumentSummary(models.Model):
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    enrollment = models.ForeignKey(Enrollment, on_delete=models.CASCADE, related_name='summaries')
    title = models.CharField(max_length=200)
    documents = models.ManyToManyField(UploadedDocument, related_name='summaries')
    content = models.TextField(blank=True)
...
class SharedResource(models.Model):
    RESOURCE_TYPES = [
        ('summary', 'Resumen'),
        ('simulation', 'Simulación de Parcial'),
        ('flashcards', 'Flashcards (Documento)')
    ]
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    group = models.ForeignKey(StudyGroup, on_delete=models.CASCADE, related_name='shared_resources')
    shared_by = models.ForeignKey(User, on_delete=models.CASCADE)
    resource_type = models.CharField(max_length=20, choices=RESOURCE_TYPES)
    message = models.ForeignKey(GroupMessage, on_delete=models.CASCADE, null=True, blank=True, related_name='attachments')
    
    # Optional references depending on resource_type
    summary = models.ForeignKey(DocumentSummary, on_delete=models.CASCADE, null=True, blank=True)
    simulation = models.ForeignKey(ExamSimulation, on_delete=models.CASCADE, null=True, blank=True)
    document_for_flashcards = models.ForeignKey(UploadedDocument, on_delete=models.CASCADE, null=True, blank=True)
